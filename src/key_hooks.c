...
{
	if (tile == FLOOR)
		mlx_image_to_window(ptr->mlx, ptr->floor.img_ptr,
			x * TILE_SIZE, y * TILE_SIZE);
	else if (tile == WALL)
		mlx_image_to_window(ptr->mlx, ptr->wall.img_ptr,
			x * TILE_SIZE, y * TILE_SIZE);
	else if (tile == COLLECT)
		mlx_image_to_window(ptr->mlx, ptr->collectible.img_ptr,
			x * TILE_SIZE, y * TILE_SIZE);
	else if (tile == EXIT)
	{
		if (ptr->map.collectible == 0)
			mlx_image_to_window(ptr->mlx, ptr->open_exit.img_ptr,
				x * TILE_SIZE, y * TILE_SIZE);
		else
			mlx_image_to_window(ptr->mlx, ptr->exit.img_ptr,
				x * TILE_SIZE, y * TILE_SIZE);
	}
	else if (tile == PLAYER)
		mlx_image_to_window(ptr->mlx, ptr->player.img_ptr,
			x * TILE_SIZE, y * TILE_SIZE);
}

void	player_movement(t_solong *ptr, int current_x, int current_y)
{
	if (current_x < 0 || current_x >= ptr->map.columns
		|| current_y < 0 || current_y >= ptr->map.rows
		||ptr->map.map[current_y][current_x] == WALL
		|| (ptr->map.map[current_y][current_x] == EXIT
